...
func (b *Bot) resolveTaskID(msg *tgbotapi.Message, partialID, mode string) (minuano.Task, bool) {
	chatID := msg.Chat.ID
...
func (b *Bot) showTaskPicker(msg *tgbotapi.Message, tasks []minuano.Task, mode, project string) {
	chatID := msg.Chat.ID
	threadID := getThreadID(msg)
	userID := msg.From.ID
